	...
	<section>
		<b-collapse class='panel' :open.sync='isOpen' >
			<div class='panel-heading' slot='trigger' role='button'>
				<strong>Favorite Rolls</strong>
			</div>
			<div class='panel-block'>
				<div class="columns is-multiline is-mobile">
					<div class='column' v-for='diceRoll in diceRolls' :key='diceRoll'>
						<b-button type='is-primary' size='is-large' @click='roll(diceRoll)'><vue-fontawesome :icon="['fas','dice-d20']"/>&nbsp; {{ diceRoll }} ({{ diceRollResults[diceRoll] }})</b-button>
					</div>
					<div class='column' v-show='noDiceRolls'>There are no dice rolls in favorites.</div>
				</div>
			</div>
		</b-collapse>
	</section>
